...
class 技能2(主动技能):  # 雷霆踏
    ...
    等级精通 = 60
    MP = [20, 146]
    data0 = [int(i*1.05) for i in [0, 4444, 4893, 5345, 5796, 6247, 6697, 7148, 7599, 8050, 8501, 8951, 9402, 9853, 10304, 10754, 11205, 11656, 12108, 12558, 13009, 13460, 13911, 14361, 14812, 15263, 15714, 16164, 16615, 17066, 17517, 17967, 18418, 18869, 19321,
    ...
    hit0 = 1
    感电data0 = [int(i*1.05) for i in [0, 548, 604, 659, 716, 772, 828, 883, 939, 995, 1050, 1106, 1162, 1217, 1273, 1328, 1384, 1440, 1495, 1551, 1608, 1664, 1719, 1775, 1830, 1886, 1942, 1997, 2053, 2109, 2164, 2220, 2276, 2331,
...
class 技能3(主动技能):  # 念气波
    ...
    等级精通 = 60
    MP = [15, 154]
    data0 = [int(i*1.053) for i in [0, 1693, 1864, 2037, 2208, 2379, 2552, 2723, 2895, 3067, 3239, 3410, 3581, 3754, 3925, 4097, 4269, 4441, 4612, 4785, 4956, 5128, 5300, 5471, 5643, 5815, 5987, 6158, 6331, 6502, 6674, 6846, 7018, 7189, 7361,
    ...
    hit0 = 1
    感电data0 = [int(i*1.053) for i in [0, 188, 207, 226, 245, 264, 282, 303, 322, 340, 359, 378, 397, 416, 436, 455, 474, 493, 512, 531, 550, 570, 589, 608, 626, 645, 664, 683, 703, 722, 741, 760, 779, 798, 818,
...
class 技能5(主动技能):  # 幻影爆碎 TP加分身个数
    ...
    等级精通 = 60
    是否主动 = 0
    data0 = [int(i*1.051) for i in [0, 873, 962, 1050, 1139, 1228, 1317, 1405, 1494, 1583, 1671, 1760, 1849, 1938, 2026, 2115, 2204, 2292, 2381, 2469, 2557, 2646, 2735, 2824, 2912, 3001, 3090, 3179, 3267, 3356, 3445, 3533, 3622, 3711,
...
class 技能7(主动技能):
    ...
    MP = [5, 210]
    关联技能 = ['所有']
    data0 = [int(i*1.052) for i in [0, 60, 70, 79, 90, 99, 109, 118, 128, 139, 148, 157, 167, 176, 187, 197,
...
class 技能8(主动技能):  # 念气环绕：袭
    ...
    等级精通 = 1
    MP = [75, 630]
    data0 = [int(i*1.05) for i in [0, 18993, 19397, 19803, 20208,
...
class 技能11(主动技能):  # 狮子吼
    ...
    data0 = [int(i*1.05) for i in [0, 6693, 7372, 8051, 8730, 9409, 10088, 10767, 11446, 12125, 12804, 13483, 14162, 14841, 15520, 16199, 16878, 17558, 18236, 18916, 19594, 20274, 20952, 21632, 22310, 22990, 23668, 24348, 25027, 25706, 26385, 27064, 27743, 28422,
...
class 技能12(主动技能):  # 螺旋念气场
    ...
    data0 = [int(i*1.05) for i in [0, 606, 668, 729, 791, 852, 914, 976, 1037, 1099, 1160, 1222, 1283, 1345, 1407, 1468, 1531, 1592, 1654, 1716, 1777, 1839, 1900, 1962, 2023, 2085, 2147, 2208, 2270, 2331, 2393, 2454, 2516, 2578,
    ...
    hit0 = 17
    风雷啸 = 1.70  # 风雷啸
    感电data0 = [int(i*1.05) for i in [0, 271, 298, 326, 354, 381, 408, 437, 464, 491, 519, 547, 574, 601, 629, 657, 684, 712, 739, 767, 794, 822, 850, 877, 905, 932, 960, 987, 1015, 1042, 1070, 1098, 1125, 1153, 1180, 1208,
...
class 技能14(主动技能):  # 念兽：猛虎震地  CP武器 爆炸=多段（多段*2）
    ...
    data0 = [int(i) for i in [0, 2245, 2473, 2701, 2929, 3157, 3385, 3612, 3840, 4068, 4296, 4524, 4751, 4980, 5207, 5436, 5663, 5890, 6119, 6346, 6575, 6802, 7031, 7258, 7485, 7714, 7941, 8170, 8397, 8625, 8853, 9081, 9309, 9536, 9764, 9992, 10220,
    ...
    hit0 = 4
    data1 = [int(i) for i in [0, 8983, 9895, 10806, 11717, 12629, 13540, 14452, 15362, 16274, 17186, 18097, 19008, 19920, 20831, 21743, 22653, 23565, 24477, 25389, 26299, 27211, 28122, 29034, 29945, 30856, 31768, 32680, 33590, 34502, 35413, 36325, 37236, 38147,
...
class 技能16(主动技能):  # 念兽：审判之金雷虎 一觉
    ...
    # 金雷虎身体攻击力：<data0>%
    data0 = [int(i*1.053) for i in [0, 2315, 2852, 3389, 3926, 4463, 5000, 5537, 6074, 6611, 7148, 7685, 8222, 8759, 9296, 9833, 10370, 10907, 11444, 11981,
    ...
    hit0 = 6
    # 金雷虎落地时的冲击波攻击力：<data1>%
    data1 = [int(i*1.053) for i in [0, 6286, 7742, 9200, 10658, 12115, 13573, 15031, 16488, 17946, 19404, 20861, 22319, 23775, 25233, 26691, 28148, 29606, 31064,
    ...
    hit1 = 3
    # 金雷虎爆炸攻击力：<data2>%
    data2 = [int(i*1.053) for i in [0, 16872, 20784, 24696, 28609, 32522, 36434, 40347, 44259, 57806, 62499, 67196, 71891, 76584, 81281, 85976, 90669, 95364, 100060, 104754,
...
class 技能17(主动技能):  # 念之战矛
    ...
    data0 = [int(i*1.05) for i in [3417, 3763, 4110, 4456, 4803, 5150, 5496, 5843, 6189, 6536, 6883, 7229, 7576, 7923, 8269, 8616, 8962, 9309, 9656, 10002, 10350, 10697, 11043, 11390,
    ...
    hit0 = 1
    data1 = [int(i*1.05) for i in [13667, 15054, 16440, 17828, 19214, 20601, 21987, 23374, 24760, 26147, 27533, 28920, 30307, 31694, 33080, 34467, 35853, 37240, 38626, 40013, 41400, 42787, 44173,
    ...
    hit1 = 1
    感电data0 = [int(i*1.05) for i in [12, 13, 14, 15, 17, 18, 19, 20, 21, 22, 24, 25, 26, 27, 28, 30, 31, 32, 33, 34, 35,
...
class 技能18(主动技能):  # 冲云念气场
    ...
    data0 = [int(i*1.052) for i in [0, 3966, 4369, 4770, 5173, 5575, 5978, 6380, 6783, 7185, 7587, 7989, 8392, 8794, 9197, 9599, 10001, 10403, 10806, 11209, 11611, 12014, 12416, 12818, 13220,
    ...
    hit0 = 6
    感电data0 = [int(i) for i in [0, 490, 540, 590, 638, 688, 738, 788, 837, 887, 937, 987, 1036, 1086, 1136, 1186, 1235, 1285, 1335, 1385, 1434, 1484, 1534, 1584, 1633,
...
class 技能19(主动技能):  # 奔雷螺旋击
    ...
    data0 = [int(i*1.054) for i in [0, 55923, 61596, 67270, 72943, 78617, 84290, 89964, 95638, 101311, 106983, 112657, 118330, 124004, 129678, 135351, 141025, 146698, 152372, 158045, 163718, 169391, 175065, 180738, 186412, 192085, 197759, 203432, 209106, 214779, 220452, 226125, 231799, 237472, 243146,
...
class 技能20(主动技能):  # 月辉念气破
    ...
    data0 = [int(i*1.051) for i in [0, 7404, 8154, 8906, 9657, 10409, 11160, 11910, 12662, 13413, 14165, 14915, 15666, 16418, 17169, 17921, 18671, 19422, 20174,
...
class 技能22(主动技能):  # 月华万象 二觉
    ...
    data0 = [int(i*1.054) for i in [0, 5142, 6334, 7527, 8719, 9912, 11103, 12296, 13488, 14681, 15873, 17066, 18258, 19451, 20643, 21836, 23028, 24221, 25412, 26605, 27797, 28990, 30182, 31375,
    ...
    hit0 = 12
    data1 = [int(i*1.054) for i in [0, 107988, 133028, 158069, 183109, 208151, 233191, 258231, 283272, 308312, 333353, 358394, 383435, 408475, 433516, 458556, 483598, 508638, 533679, 558719, 583760, 608800, 633842, 658882, 683923,
...
class 技能24(主动技能):  # 禅语·形灭
    ...
    data0 = [int(i*1.05) for i in [0, 4831, 5321, 5811, 6301, 6791, 7282, 7772, 8262, 8752, 9242, 9732, 10223, 10713, 11203, 11693, 12183, 12674, 13164, 13654, 14144, 14634, 15124, 15615, 16105,
    ...
    hit0 = 10
    data1 = [int(i*1.05) for i in [0, 48314, 53215, 58117, 63018, 67920, 72821, 77722, 82624, 87526, 92427, 97328, 102230, 107132, 112032, 116934, 121836, 126737, 131638, 136540, 141442, 146342, 151244, 156146, 161048,
...
class 技能25(主动技能):  # 禅念·归一 三觉
    ...
    data0 = [int(i*1.05) for i in [0, 310639, 382672, 454704, 526736, 598768, 670800, 742833, 814866, 886898, 958931, 1030963, 1102995, 1175027, 1247059, 1319092, 1391125, 1463157, 1535190, 1607222, 1679254, 1751286, 1823318, 1895351, 1967383,
    ...
    hit0 = 1
    data1 = [int(i*1.05) for i in [0, 34515, 42518, 50522, 58525, 66529, 74532, 82536, 90541, 98543, 106548, 114551, 122555, 130558, 138562, 146566, 154569, 162573, 170576, 178580, 186583, 194587, 202591, 210594, 218598,

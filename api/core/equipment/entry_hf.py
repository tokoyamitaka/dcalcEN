...
def entry_1227(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 1:
        if '睡眠' in own_state_type:
            ...
            char.技能攻击力加成(part=part, x=0.1*char.睡眠效果())
        pass
    ...
    if mode == 1:
        if '破招攻击' in attack_type:
            if '非破招攻击' in attack_type:
                char.技能攻击力加成(part=part, x=0.025)
            else:
                char.技能攻击力加成(part=part, x=0.05)
        pass
...
def entry_1229(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 1:
        if '睡眠' in own_state_type:
            char.攻击强化加成(params[0]*char.睡眠效果())
        pass
...
def entry_1040(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min((90-hp_rate_num)/10, 9))
...
def entry_1041(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min((90-mp_rate_num)/10, 9))
...
def entry_930(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(3, len(state_type)))
...
def entry_1018(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(4, teammate_num))
...
def entry_1019(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 1:
        char.攻击强化加成(params[0] * min(4, teammate_num))
        char.技能攻击力加成(part=part, x=0.02*min(4, teammate_num))
...
def entry_266(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(60, enemy_num))
...
def entry_139(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)
...
def entry_143(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)*(1 if '冰冻' in state_type else 0))
...
def entry_144(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)*(1 if '减速' in state_type else 0))
...
def entry_145(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)*(1 if '眩晕' in state_type else 0))
...
def entry_146(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)*(1 if '诅咒' in state_type else 0))
...
def entry_147(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)*(1 if '失明' in state_type else 0))
...
def entry_148(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)*(1 if '石化' in state_type else 0))
...
def entry_149(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * min(10, enemy_num)*(1 if '睡眠' in state_type else 0))
...
def entry_195(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0] * toughness_num)
...
def entry_224(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0]*0.5*20/30)
...
def entry_1261(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["施放技能时，每累计消耗50个无色小晶块，攻击强化 +296(最多叠加5次)"]
    if mode == 0:
        pass
    if mode == 1:
...
def entry_1262(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["施放技能时，每消耗1个无色小晶块，该技能的冷却时间恢复速度 +0.5%(觉醒技除外，最多增加30%)"]
    if mode == 0:
        pass
    if mode == 1:
        pass
...
def entry_1263(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["装备时，每秒获得1层叠加(最大10层)", "- 使用非敌人弱点属性的控制型异常时，消耗所有的叠加层数，根据消耗的层数增加韧性条的减少量(适用于正常弱点异常状态韧性槽减少量，最少50%，最大100%)"]
    if mode == 0:
        pass
    if mode == 1:
        pass
...
def entry_1264(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 0:
        pass
    if mode == 1:
        char.攻击强化加成(params[0]*min(10, toughness_num))
        pass
...
def entry_1266(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["攻击时，物理、魔法防御力 +10000，效果持续20秒(冷却时间5秒，最多叠加1次)"]
    if mode == 0:
        pass
    if mode == 1:
        pass
...
def entry_1269(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["加成最高的伤害型异常状态的伤害加成数值累计55%以上时，技能攻击力+13%"]
    if mode == 0:
        pass
    if mode == 1:
...
def entry_1271(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 1:
        if char.异常抗性获取('睡眠') < 0:
            char.技能攻击力加成(part=part, x=min(0-char.异常抗性获取('睡眠') * 0.4, 0.28))
        pass


def entry_1272(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["进入地下城后，每消耗10000点MP，自身进入一次睡眠状态", "睡眠抗性-10%"]
    if mode == 0:
        pass
    if mode == 1:
        char.异常抗性加成('睡眠', -0.1)
...
def entry_1273(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    ...
    if mode == 1:
        for i in char.特效:
            i["power"] *= 1.3
        char.攻击强化加成(params[0]*min(len(char.特效), 10))
        pass


def entry_1274(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["攻击时，向300px范围内所有敌人发动燃烧攻击(冷却时间5秒，无视敌人防御力，伤害为总攻击强化数值的35%)"]
    if mode == 0:
        pass
    if mode == 1:
        char.特效.append({
...
def entry_1275(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["自身每处于1种异常状态，所有属性强化 +8(最多增加24)"]
    if mode == 0:
        pass
    if mode == 1:
...
def entry_1276(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["每存在1个处于异常状态的对象，攻击强化 +445(最多叠加10次)"]
    if mode == 0:
        pass
    if mode == 1:
        count = 0
...
def entry_1280(char: CharacterProperty = {}, mode=0, text=False, part='', lv=0,params=[]):
    if text:
        return ["HP低于50%以下时，物理防御减伤率和魔法防御减伤率总和超过70%以上时，获得霸体护甲效果，持续20秒，5次被击时效果消失"]
    if mode == 0:
        pass
    if mode == 1:
        pass
